...
文档常用设问、承接和邀请阅读的方式引出扩展知识，例如：
“那么某理论有哪些内容呢？每个阶段有什么特征与主要任务呢？我们一起来看下面的知识链接吧。”

抽取时遵循以下规则：

1. 这类句子是**知识块的结构提示**，不是知识结论；其中“那么”“有哪些呢”“我们一起来看”等修辞性话语，不单独抽成实体、关系、属性，也不作为实体 `description`。
2. 从导读语中识别其指向的**规范知识主题**；例如将“埃里克森心理发展八阶段理论”识别为一个完整的“心理理论/理论框架”实体，而不是把整句设问作为实体。
3. 将导读语之后连续出现的阶段、步骤、组成部分统一挂到该主题下。理论的各阶段可建为阶段实体，并以“包含”等关系连接；阶段的年龄范围、核心冲突、主要特征、发展任务等作为属性，或在确有独立检索价值时建为相关实体。
4. 不得把同一理论框架仅因标题、导读语和正文表述不同而拆成多个同名或近义实体；应统一使用正文或权威标题中的规范名称。
5. 框架不能成为孤立的扩展知识岛：若原文上下文明确它用于解释正文概念，应保留与正文核心概念的语义关系，例如“埃里克森心理发展八阶段理论 —解释→ 同一性危机”。
6. 若导读语本身包含可验证的实质性断言，只抽取该断言；去除设问、感叹、邀请阅读等教学话术。实体 `description` 仍只能逐字摘录真正描述该实体的原句，不得把提炼后的总结写入 `description`。
